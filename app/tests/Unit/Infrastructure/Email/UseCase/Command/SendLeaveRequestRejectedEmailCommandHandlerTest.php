...
beforeEach(function (): void {
    ...
    $this->handler = new SendLeaveRequestRejectedEmailCommandHandler(
...
it('uses correct template', function () {
    ...
    $this->mailer
        ...
        ->once()
        ->withArgs(function (TemplatedEmail $email) {
            expect($email->getHtmlTemplate())->toBe('@AppEmail/leave_request_rejected.html.twig');
...
it('includes rejector name in context', function () {
    $rejector = UserDTOFixture::create(['firstName' => 'Jane', 'lastName' => 'Manager']);
    $leaveRequestDTO = LeaveRequestDTOFixture::create(['approvedBy' => $rejector]);
    ...
    $this->mailer
        ...
        ->once()
        ->withArgs(function (TemplatedEmail $email) {
            expect($email->getContext()['rejected_by'])->toBe('Jane Manager');
