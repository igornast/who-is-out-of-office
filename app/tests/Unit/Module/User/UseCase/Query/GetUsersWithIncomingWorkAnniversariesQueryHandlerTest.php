...
beforeEach(function (): void {
    ...
    $this->handler = new GetUsersWithIncomingWorkAnniversariesQueryHandler(userRepository: $this->userRepository);
});

it('finds users with incoming work anniversaries using default 20 days period', function () {
    $expectedUsers = [
...
it('finds users with incoming work anniversaries using custom end date', function () {
...
it('returns empty array when no users have upcoming work anniversaries', function () {
    $this->userRepository
        ->expects('findUsersWithIncomingWorkAnniversaries')
        ->once()
        ->andReturn([]);
